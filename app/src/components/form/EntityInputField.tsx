...
interface EntityInputFieldProps<T extends LocalStoreEntityType> {
  className?: string;
  name: keyof T & string;
  state: StateType<T>;
  label?: string;
...
function EntityInputField<T extends LocalStoreEntityType>({
  className,
  name,
  state,
  label,
}: EntityInputFieldProps<T>) {
  return (
...
export default styled(EntityInputField)`
  position: relative;
  width: 100%;
